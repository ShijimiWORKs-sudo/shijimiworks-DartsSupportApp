...
- レベル別デイリーミニマム、BULL/クリケット/シングル/ダブル/トリプル/フォーム/プレッシャーの独自ドリルを実施・記録する
- 5〜60分の時間別プリセットと、過去成績に基づく理由付きおすすめ候補を表示する
- SQLite データをPC分析用メタ情報付きJSONで Export/Import する
- PC Web版でiPhoneのJSONを取り込み、履歴、BULL、クリケット、キャッチ、レベル、フォーム改善を分析する
- PC Web版で分析用CSVをUTF-8 BOM付きでコピー出力する
...
PCブラウザ版は、iPhoneで記録したJSONを取り込む分析用画面です。クラウド同期や正式なゲームエンジンは実装せず、次の流れを想定します。

1. iPhone版の「バックアップ」でJSONをExportする。
2. JSONファイルをPCへ移動する。
3. PC Web版の「PC分析」でJSONを選択し、プレビュー後にImportする。
4. IndexedDBへ永続保存されたデータで、履歴、BULL率、目標達成までの投矢数、クリケット平均マーク、キャッチ、得意・苦手候補、レベル推移、フォーム改善を確認する。

Webだけブラウザ標準のIndexedDBを使用します。iOS/Androidは引き続きSQLite repositoryを使用します。今回のPC分析実装で新しい依存パッケージは追加していません。
